...
func _ready() -> void:
	var fog: bool = Settings.graphics_get_volumetric_fog()
	...
	var filtering: bool = Settings.graphics_get_volumetric_fog_filtering()
...
func _on_volumetric_fog_options_item_selected(index: int) -> void:
	Settings.graphics_set_volumetric_fog(index == 1)
	toggle_submenu(index == 1)

func _on_volumetric_fog_filtering_options_item_selected(index: int) -> void:
	Settings.graphics_set_volumetric_fog_filtering(index == 1)
